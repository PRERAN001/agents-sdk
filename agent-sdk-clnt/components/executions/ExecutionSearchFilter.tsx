...
interface ExecutionSearchFilterProps {
  search: string;
...
export default function ExecutionSearchFilter({
  search,
  onSearchChange,
  statusFilter,
  onStatusChange,
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="relative w-full sm:w-80">
          ...
          <Input
            type="text"
            placeholder="Search task, ID, or runtime..."
            value={search}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between pt-2 border-t border-zinc-200 dark:border-zinc-800 text-xs text-zinc-500">
          <span>
            Showing page <strong>{page}</strong> of <strong>{totalPages}</strong> ({total} total executions)
          </span>
